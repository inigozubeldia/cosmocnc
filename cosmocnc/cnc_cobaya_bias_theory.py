...
class cnc(classy):
    ...
    survey_sr: Optional[str] =  "surveys/survey_sr_so_sim.py" #File where the survey scaling relations are defined
    survey_cat: Optional[str] =  "surveys/survey_cat_so_sim.py" #File where the survey catalogue(s) are defined
    load_catalogue: Optional[bool] = False
...
#    "observables": [["q_mmf3_mean"]],
    ...
    # bin edges for bias calculation
    zc_edges: Optional[str] = np.array([0.2, 0.4, 0.55, 0.7, 0.85])
    ...
    def initialize(self):
        ...
        #Observables and catalogue
        self.cnc.cnc_params["load_catalogue"] = self.load_catalogue 
    ...
    def get_biases(self):
        ...
        cls = deepcopy(self._current_state["biases"])
        # print('cls',cls)
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        # ACT parameters:
        assign_parameter_value(scal_rel_params,params_values,"A0")
        assign_parameter_value(scal_rel_params,params_values,"B0")
        assign_parameter_value(scal_rel_params,params_values,"C0")
        assign_parameter_value(scal_rel_params,params_values,"sigma_lnq_act")
        ...
        bias_vec = np.zeros(len(self.zc_edges) - 1)
        for i in range(len(self.zc_edges) - 1):

            self.cnc.cnc_params.update({
                    "z_min": self.zc_edges[i],
                    "z_max": self.zc_edges[i+1],
                    "bins_edges_z": np.linspace(self.zc_edges[i], \
                                    self.zc_edges[i+1], self.cnc.cnc_params["n_z"])
            })

            self.cnc.update_params(cosmo_params, scal_rel_params, self.cnc.cnc_params)

            self.cnc.get_number_counts() #Evaluate the binned likelihood
            self.cnc.get_bias_weighted_cluster_abundance() #Evaluate the binned likelihood

            n_tot_bin = self.cnc.n_tot
            n_tot_bias_weighted = self.cnc.n_tot_bias_weighted

            bias_mean = n_tot_bias_weighted/n_tot_bin

            bias_vec[i] = bias_mean

        state["biases"] = bias_vec
