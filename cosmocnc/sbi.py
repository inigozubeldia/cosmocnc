...
class sbi_cnc:
    ...
    def __init__(self,params_sbi=None):
        ...
        self.inference = SNPE(prior=prior,density_estimator=density_estimator_build_fun)

    def train(self,save=False):

        data_train = data_sbi(params_sbi=self.params_sbi)

        data = data_train.data
        params = data_train.params
        n_sim = data_train.n_sim
        ...
        density_estimator = self.inference.train()#stop_after_epochs=300,max_num_epochs=300)
    ...
    def load(self):
        ...
        self.posterior = np.load("posterior_" + params_sbi["data_train"] + ".npy",allow_pickle=True)[()]
    ...
    def __init__(self,params_sbi=params_sbi_default):
        ...
        if params_sbi["data_train"] == "so_szcmblens_train":
            ...
            for i in range(0,len(sim_ids)):
                ...
                #prename = "/rds-d4/user/iz221/hpc-work/catalogues_so_sbi/scores/catalogue_scores_train_paper_it1_"
            ...
            np.save("data_sbi_hprior.npy",(data,params))

            (data,params) = np.load("data_sbi_hprior.npy",allow_pickle=True)[()]
