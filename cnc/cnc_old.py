...
class cluster_number_counts:
    ...
    def get_cluster_abundance(self):
        ...
        if self.cnc_params["parallelise_type"] == "patch":
            ...
            indices_split_patches = np.array_split(np.arange(self.n_patches),self.cnc_params["number_cores_abundance"])
            indices_split_redshift = [np.arange(len(self.redshift_vec)) for i in range(0,self.cnc_params["number_cores_abundance"])]

        elif self.cnc_params["parallelise_type"] == "redshift":

            indices_split_patches = [np.arange(self.n_patches) for i in range(0,self.cnc_params["number_cores_abundance"])]
            indices_split_redshift = np.array_split(np.arange(len(self.redshift_vec)),self.cnc_params["number_cores_abundance"])
    ...
    def get_log_lik_data(self):
        ...
        if len(indices_other_obs) > 0:
            ...
            indices_split = np.array_split(indices_other_obs,self.cnc_params["number_cores_abundance"])
